...
const DataManager = {
    ...
    bindEvents() {
        ...
        if (btnWebdavSync) {
            btnWebdavSync.addEventListener('click', async () => {
        ...
        if (webdavUrl) {
            webdavUrl.addEventListener('change', () => {
                this.saveWebDAVSettings();
            });
        }
        if (webdavUser) {
            webdavUser.addEventListener('change', () => {
                this.saveWebDAVSettings();
            });
        }
        if (webdavPass) {
            webdavPass.addEventListener('change', () => {
                this.saveWebDAVSettings();
            });
        }
    ...
    async testWebDAVConnection() {
        ...
        try {
            ...
            if (result.ok) {
                document.getElementById('syncStatus').textContent = i18n.t('webdavConnected');
            ...
            document.getElementById('syncStatus').textContent = i18n.t('webdavConnectionFailed');
    ...
    async syncWebDAV() {
        ...
        try {
            ...
            if (result.ok) {
                ...
                await DB.saveSetting('webdavSyncEnabled', true);
            } else {
